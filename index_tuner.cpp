...
std::vector<AbstractCandidateAssessment> IndexTuner::_assess_index_candidates(const Workload& workload, std::vector<IndexCandidate>& index_candidates) {
  ...
  for (auto& index_candidate : index_candidates) {
    ...
    _output << table_name << " " << std::to_string(column_id) << " Rows Processed: " << std::to_string(total_processed_rows) << " Predicted Index Size: " << std::to_string(predict_index_size(table_name, column_id)) << std::endl;
  }
