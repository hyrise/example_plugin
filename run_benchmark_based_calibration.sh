...
do
	for i in {0..5}
	do
		/usr/bin/time -v --output=${BENCHMARK}__${ENCODING}.time sh -c "BENCHMARK_TO_RUN=${BENCHMARK} ENCODING_CONFIG=random_configs/random_config__${BENCHMARK}__${i}.json numactl -m 3 -N 3 ./${FOLDER}/hyrisePlayground ${FOLDER}/${PLUGIN}"
		# retVal=$?
		# if [ $retVal -ne 0 ]; then
