...
std::vector<AbstractCandidateAssessment> IndexTunerAdvanced::_assess_index_candidates(const Workload& workload, std::vector<IndexCandidate>& index_candidates) const {
  ...
  for (auto& index_candidate : index_candidates) {
    ...
    std::cout << table_name << " " << std::to_string(column_id) << " Cost Reduction: " << std::to_string(cost_reduction) << " Predicted Index Size: " << std::to_string(predict_index_size(table_name, column_id)) << std::endl;
  }
